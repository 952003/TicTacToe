...
const disableCell = index => $('#gameboard #${index}').prop("disable", true);
...
const turnOffOnSuccess = (elem) => elem.hasClass("text-success") ? elem.removeClass("text-success");
...
hubConnection.on("OnMoveMaid", index => {
    ...
    turnOffOnSuccess($('h3[data-id=${myConnectionId}]'));
    turnOffOnSuccess($('h3[data-id!=${nyConnectionId}]'));
    isMyMove = !isMyMove;
});
